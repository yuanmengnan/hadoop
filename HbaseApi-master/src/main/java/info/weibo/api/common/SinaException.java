...
public class SinaException extends RuntimeException {

	/**
	 * 序列号
	 */
	private static final long serialVersionUID = -5145999839675254548L;

	/**
	 * This list of valid HTTP Status error codes that Sina may return in 
	...
	public enum ErrorCode {
		...
		BAD_REQUEST(400), UNAUTHORIZED(401), FORBIDDEN(403), NOT_FOUND(404), CONFLICT(409), UNSUPPORTED_MEDIA_TYPE(415), SERVER_ERROR(
				500), SERVICE_UNAVAILABLE(503), UNKNOWN(0);
	...
	public SinaException(ErrorCode code, String msg) {
		super(msg);
		this.code = code.code;
	}

	public SinaException(ErrorCode code, String msg, Throwable th) {
		super(msg, th);
		this.code = code.code;
	}

	public SinaException(ErrorCode code, Throwable th) {
		super(th);
		this.code = code.code;
	}
	 ...
	 * use in Sina, but may be used by clients in subclasses to capture 
	 * errors returned by the servlet container or other HTTP proxies.
	 */
	protected SinaException(int code, String msg, Throwable th) {
		super(msg, th);
		this.code = code;
	}
	...
	/**
	 * The HTTP Status code associated with this Exception.  For SinaExceptions 
	 * thrown by Sina "Server Side", this should valid {@link ErrorCode}, 
	 * however client side exceptions may contain an arbitrary error code based 
	 * on the behavior of the Servlet Container hosting Sina, or any HTTP 
